...
- (void)toggleState {
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:@"/var/mobile/Library/Preferences/love.litten.dresspreferences.plist"]) {
        enableMediaPlayerSection = YES;
        [preferences setBool:enableMediaPlayerSection forKey:@"EnableMediaPlayerSection"];
        [self toggleCellState:YES];
    } else if (![allKeys containsObject:@"EnableMediaPlayerSection"]) {
        enableMediaPlayerSection = YES;
        [preferences setBool:enableMediaPlayerSection forKey:@"EnableMediaPlayerSection"];
        [self toggleCellState:YES];
    } else if ([[preferences objectForKey:@"EnableMediaPlayerSection"] isEqual:@(NO)]) {
        enableMediaPlayerSection = YES;
        [preferences setBool:enableMediaPlayerSection forKey:@"EnableMediaPlayerSection"];
        [self toggleCellState:YES];   
    } else if ([[preferences objectForKey:@"EnableMediaPlayerSection"] isEqual:@(YES)]) {
        enableMediaPlayerSection = NO;
        [preferences setBool:enableMediaPlayerSection forKey:@"EnableMediaPlayerSection"];
